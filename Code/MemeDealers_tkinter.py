...
def get_random_meme(memeList, meme_ind, label, user_inp):
...
def last_meme(memeList, meme_ind, label, user_inp, konami_code):
    meme_ind[0] -= 1
    if meme_ind[0] < 0:
        meme_ind[0] = 0
    meme = memeList[meme_ind[0]]
...
def copyToClipboard(meme):
    ...
    clipboard.clipboard_get(type=meme)
    print("Clipboard")
...
def share(check, website):
    ...
    """
    if check[0] == 0:
        meme = last_meme(memes, meme_ind, back_forward_check, image_label, user_inp=[], konami_code=0)
    else:
        meme = get_random_meme(memes, meme_ind, back_forward_check, image_label, user_inp=[])
...
if __name__ == "__main__":
    ...
    btn = Button(btn_frame, text='Get Meme', command=lambda: get_random_meme(memes, meme_ind, image_label, user_inp) if og == True else get_random_meme(ai_memes, ai_ind, image_label, user_inp))
    back_btn = Button(btn_frame, text='Last Meme', command=lambda: last_meme(memes, meme_ind, image_label, user_inp,konami_code) if og == True else last_meme(ai_memes, ai_ind, image_label, user_inp, konami_code))
